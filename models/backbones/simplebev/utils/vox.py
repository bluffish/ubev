...
class Vox_util(object):
    ...
    def Ref2Mem(self, xyz, Z, Y, X, assert_cube=False):
        ...
        xyz = models.backbones.simplebev.utils.geom.apply_4x4(mem_T_ref, xyz)
        return xyz
    ...
    def Mem2Ref(self, xyz_mem, Z, Y, X, assert_cube=False):
        ...
        xyz_ref = models.backbones.simplebev.utils.geom.apply_4x4(ref_T_mem, xyz_mem)
        return xyz_ref
    ...
    def get_mem_T_ref(self, B, Z, Y, X, assert_cube=False, device='cuda'):
        ...
        center_T_ref = models.backbones.simplebev.utils.geom.eye_4x4(B, device=device)
        ...
        mem_T_center = models.backbones.simplebev.utils.geom.eye_4x4(B, device=device)
        mem_T_center[:, 0, 0] = 1. / vox_size_X
        mem_T_center[:, 1, 1] = 1. / vox_size_Y
        mem_T_center[:, 2, 2] = 1. / vox_size_Z
        mem_T_ref = models.backbones.simplebev.utils.basic.matmul2(mem_T_center, center_T_ref)
    ...
    def unproject_image_to_mem(self, rgb_camB, pixB_T_camA, camB_T_camA, Z, Y, X, assert_cube=False, xyz_camA=None):
        ...
        if xyz_camA is None:
            xyz_memA = models.backbones.simplebev.utils.basic.gridcloud3d(B, Z, Y, X, norm=False, device=pixB_T_camA.device)
            xyz_camA = self.Mem2Ref(xyz_memA, Z, Y, X, assert_cube=assert_cube)

        xyz_camB = models.backbones.simplebev.utils.geom.apply_4x4(camB_T_camA, xyz_camA)
        z = xyz_camB[:, :, 2]

        xyz_pixB = models.backbones.simplebev.utils.geom.apply_4x4(pixB_T_camA, xyz_camA)
        ...
        if (0):
            ...
            y_pixB, x_pixB = models.backbones.simplebev.utils.basic.normalize_grid2d(y, x, H, W)
    ...
    def warp_tiled_to_mem(self, rgb_tileB, pixB_T_camA, camB_T_camA, Z, Y, X, DMIN, DMAX, assert_cube=False):
        ...
        z_tileB, y_pixB, x_pixB = models.backbones.simplebev.utils.basic.normalize_grid3d(z_tileB, y, x, D, H, W)
    ...
    def xyz2circles(self, xyz, radius, Z, Y, X, soft=True, already_mem=True, also_offset=False, grid=None):
        ...
        if grid is None:
            grid_z, grid_y, grid_x = models.backbones.simplebev.utils.basic.meshgrid3d(B, Z, Y, X, stack=False, norm=False, device=xyz.device)
    ...
    def xyz2circles_bev(self, xyz, radius, Z, Y, X, already_mem=True, also_offset=False):
        ...
        grid_z, grid_x = models.backbones.simplebev.utils.basic.meshgrid2d(B, Z, X, stack=False, norm=False, device=xyz.device)
        # note the default stack is on -1
